    ...
    render() {
        return (
            <div className="text-center">
                <div className="text-muted pull-left" onClick={this.handleClickPrevious} style={{cursor: "pointer"}}>
                ...
                <div className="text-muted pull-right" onClick={this.handleClickNext} style={{cursor: "pointer"}}>
...
const TimeInput = React.createClass({
    ...
    handleHourChange(event) {
        if(this.props.selectedDate) {
            let newSelectedDate = new Date(this.props.selectedDate);
            newSelectedDate.setHours(event.target.value);
    ...
    handleMinuteChange(event) {
        if(this.props.selectedDate) {
            let newSelectedDate = new Date(this.props.selectedDate);
            newSelectedDate.setMinutes(event.target.value);
            this.props.onChange(newSelectedDate);
        }
    },
    handleFormatChange(event) {
        if(this.props.selectedDate) {
            let newSelectedDate = new Date(this.props.selectedDate);
            let hours = newSelectedDate.getHours();
            if(event.target.value == "pm" && hours<12) hours = hours+12;
            if(event.target.value == "am" && hours==12) hours = hours-12;
    ...
    renderOption(option, key) {
        return (
            <option value={option.value} key={key}>{option.display}</option>
        )
    },
    render() {
        let currentHour = 12, currentMinute = 0, currentFormat = 'am';
        if(this.props.selectedDate) {
            let newSelectedDate = new Date(this.props.selectedDate);
            currentHour = newSelectedDate.getHours();
            currentMinute = newSelectedDate.getMinutes();
            if(currentHour > 12) {
                currentFormat = 'pm';
            }
                ...
                <div className="row">
                    <div className="col-xs-4">
                        <div className="form-group">
                            ...
                            <select name="hour" className="form-control" value={currentHour} style={{padding: "6px 12px", textAlign: 'center'}} onChange={this.handleHourChange}>
                                {hours.map(this.renderOption)}
                            </select>
                        </div>
                    </div>
                    <div className="col-xs-4">
                        <div className="form-group">
                            ...
                            <select name="minutes" className="form-control" value={currentMinute} style={{padding: "6px 12px", textAlign: 'center'}} onChange={this.handleMinuteChange}>
                                {minutes.map(this.renderOption)}
                            </select>
                        </div>
                    </div>
                    <div className="col-xs-4">
                        <div className="form-group">
                            ...
                            <select name="format" className="form-control" value={currentFormat} style={{padding: "6px 12px", textAlign: 'center'}} onChange={this.handleFormatChange}>
...
export default React.createClass({
    ...
    makeInputValueString(date) {
        ...
        let currentFormat = 'am';
        if(currentHour > 12) {
            currentFormat = 'pm';
        }
        currentHour = currentHour % 12 || 12;
        const hour = (currentHour<10?'0':'') + currentHour;
        ...
        if(this.props.dateFormat.match(/MM.DD.YYYY/)) {
            display = (month > 9 ? month : "0" + month) + separator + (day > 9 ? day : "0" + day) + separator + date.getFullYear();
        } else if(this.props.dateFormat.match(/DD.MM.YYYY/)) {
            display = (day > 9 ? day : "0" + day) + separator + (month > 9 ? month : "0" + month) + separator + date.getFullYear();
        } else {
            display = date.getFullYear() + separator + (month > 9 ? month : "0" + month) + separator + (day > 9 ? day : "0" + day);
        }
        if(this.props.timeFormat) {
            display += ' ' + hour + ':' + minutes + ' ' + currentFormat;
        }
        return display;
    },
    ...
    onApplyDate() {
        ...
        if(this.props.onChange) {
            this.props.onChange(this.state.selectedDate.toISOString());
        }
    },
    ...
    renderTime() {
        if(this.props.timeFormat) {
            return (
                <div>
                    <TimeInput selectedDate={this.state.selectedDate} onChange={this.onChangeDate} />
                    <button className="btn btn-xs btn-block btn-primary" onClick={this.onApplyDate}>Apply</button>
                </div>
            )
        }
    },
